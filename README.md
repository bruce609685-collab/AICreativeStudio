...
一个外壳式 AI 影音创作桌面工具：主程序本身**不内置任何模型调用逻辑**，生图、生视频、生语音的能力全部来自 `scripts/` 目录下的 Python 脚本。界面只负责两件事——把参数喂给脚本、把产物接回来展示。
...
- **三类创作页签**：AI 生图片 / AI 生视频 / AI 生语音，参数区按所选脚本的能力动态显隐（分辨率、画幅、时长、采样等）
...
1. 打开「模型设置」页，为你想用的脚本填入 API KEY
2. （可选）在「智能导入」页配置你的 LLM 接口地址与密钥
3. 回到生成页签，写提示词、选参数、点生成
