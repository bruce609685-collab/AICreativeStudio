...
> ⚠️ **当前为 v0.5.1 版**：还有很多 BUG，界面也很粗糙，欢迎提交 Issue / PR 帮忙改进！
...
**智能导入** —— 粘贴模型厂商的 API 接入文档，LLM 自动解析并生成脚本：
...
| v0.5 | 修复 9 处底层缺陷（导入脚本中文名丢失、文件名双 .py、配置多余键崩溃、FFmpeg 路径与下载进度、历史库跨线程写入等），补齐进度条/滑块/工具提示/菜单/分隔条统一样式；修复依赖项误列 none 假依赖 |
| v0.5.1 | 修复视频生成超时：提交上限 30 秒→120 秒、轮询单次 30 秒→60 秒；任务预算放宽到 600 秒，外壳强杀上限 660 秒 |
